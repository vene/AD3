...
def general_graph(unaries, edges, edge_weights, verbose=1, n_iterations=1000,
    ...
    if isinstance(unaries, list):
        return _general_graph_multitype(unaries, edges, edge_weights, verbose, n_iterations, eta, exact)
    else:
        return _general_graph          (unaries, edges, edge_weights, verbose, n_iterations, eta, exact)
...
def _general_graph_multitype(l_unaries, l_edges, l_edge_weights, verbose=1, n_iterations=1000,
                  eta=.1, exact=False):
    """
    ...
    for typ_i, n_states_i in enumerate(l_n_states):
        ...
        for typ_j, n_states_j in enumerate(l_n_states):
            ...
            for i, e in enumerate(edges):
                edge_variables = [multi_variables[var_start_i+e[0]], multi_variables[var_start_j+e[1]]]
                factor_graph.create_factor_dense(edge_variables,
                                                 edge_weights[i].ravel())
    ...
    for (_n_nodes, _n_states) in zip(l_n_nodes, l_n_states):  #iteration by type
        ...
        ret_node_marginals.append( marginals[ k:k+_n_marg ].reshape(_n_nodes, _n_states) )
        k += _n_marg
    ...
    for typ_i, n_states_i in enumerate(l_n_states):
        for typ_j, n_states_j in enumerate(l_n_states):
            ...
            if _n_edges:
                ret_edge_marginals.append( edge_marginals[ i_marg_start : i_marg_start + _n_marg ].reshape((_n_edges, _n_edge_states)) )
            else:
